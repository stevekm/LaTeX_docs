...
\date{November 6, 2014 \\\texttt{sk81638@gulls.salisbury.edu}} % Nov. 6 2014
...
<<AlgalSpeciesItalics,results='asis',eval=FALSE,echo=FALSE>>=
suppressPackageStartupMessages(require(xtable)) #dat[rows,coluimns]

# Species<-c('Tetraselmis chuii','Chaetoceros mulleri','Isochrysis sp.','Thalassiosira pseudonana','Nannochloropsis oculatta')
# Attributes<-c('Chlorophyta - flagellate','Ochrophyta - diatom','Haptophyta - flagellate','Ochrophyta - diatom','Ochrophyta - flagellate')
# Uses<-c('Aquaculture feed','Aquaculture feed','Aquaculture feed','Aquaculture feed','Potential for biofuel')
# Table<-cbind(Species,Attributes,Uses)
# print(xtable(Table))

library(xtable)
Species <- c("Tetraselmis chuii",
          "Chaetoceros mulleri",    
          "Isochrysis sp.",
          "Thalassiosira pseudonana",
          "Nannochloropsis oculatta")
Attributes<-c('Chlorophyta - flagellate',
              'Ochrophyta - diatom',
              'Haptophyta - flagellate',
              'Ochrophyta - diatom',
              'Ochrophyta - flagellate')
Uses<-c('Aquaculture feed',
        'Aquaculture feed',
        'Aquaculture feed',
        'Aquaculture feed',
        'Potential for biofuel')
data.df <- data.frame(Species,Attributes,Uses)
data.df$Species <- paste("\\emph{",Species,"}", sep="")
data.table <- xtable(data.df)
print(data.table, 
      sanitize.text.function=identity)

@
% editor's note: don't bother running this R code just paste the TeX table here:
